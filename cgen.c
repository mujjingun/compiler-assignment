    ...
    debugSymbolState* debugSymbols)
{
    ...
    case ExprId: {
        ...
        if (is_addr) {
            if (t->record->scope == 0) {
                ...
                switch (t->storage) {
                case Memory:
                    emitInstruction(out, "addiu $%s, $fp, %d\n", reg_name, loc);
                    break;
                default:
                    break;
                }
    ...
    case ExprIndex: {
        ...
        if (is_addr) {
            if (t->record->scope == 0) {
                ...
                case Memory:
                    if (loc < 0) {
                        emitInstruction(out, "addu  $%s, $fp, $%s\n", reg_name, idx_reg_name);
                        emitInstruction(out, "addiu $%s, $%s, %d\n", reg_name, reg_name, loc);
                    } else {
                        char other_reg_name[3];
                        register_name(Argument, loc, other_reg_name);
    ...
    case ExprCall: {
        ...
        emitInstruction(out, "addiu $sp, $sp,%d\n", -56);
        for (int i = 0; i < 10; ++i) {
            emitInstruction(out, "sw    $t%d, %d($sp)\n", i, i * 4);
        }
        for (int i = 0; i < 4; ++i) {
            emitInstruction(out, "sw    $a%d, %d($sp)\n", i, (i + 10) * 4);
        }
        ...
        for (int i = 0; i < t->num_children; ++i) {
            emitInstruction(out, "move  $a%d, $t%d\n", i, reg_num + i);
        }

        // push the control link
        emitInstruction(out, "addiu $sp,$sp,%d\n", -4);
        emitInstruction(out, "sw    $fp,0($sp) # push control link\n");

        // jump to the procedure
        emitInstruction(out, "jal   %s\n", t->value.name);

        // pop the control link
        emitInstruction(out, "sw    $fp,0($sp) # pop control link\n");
        emitInstruction(out, "addiu $sp,$sp,%d\n", 4);

        // pop temporary registers
        for (int i = 0; i < 10; ++i) {
            emitInstruction(out, "lw    $t%d, %d($sp)\n", i, i * 4);
        }
        for (int i = 0; i < 4; ++i) {
            emitInstruction(out, "lw    $a%d, %d($sp)\n", i, (i + 10) * 4);
        }
        emitInstruction(out, "addiu $sp,$sp,%d\n", 56);

        // return value
        emitInstruction(out, "move  $%s,$v0\n", ret_reg_name);
        break;
    }
...
static void cGen(Node t, codegenState state, debugSymbolState* debugSymbols)
{
    ...
    case NodeStmt: {
        ...
        case StmtWhile: {
            ...
            emitInstruction(out, "j     $_L%d # loop\n", loop_label);

            emitInstruction(out, "$_L%d: # loop exit\n", exit_label);
        ...
        case StmtFunction: {
            emitInstruction(out, "%s:\n", t->value.func.name);

            emitComment(out, "set frame pointer");
            emitInstruction(out, "move  $fp,$sp\n");

            emitComment(out, "push the return address");
            emitInstruction(out, "addiu $sp,$sp,%d\n", -4);
            emitInstruction(out, "sw    $ra,0($sp)\n\n");
        ...
        case StmtCompoundStmt: {
            ...
            emitInstruction(out, "addiu $sp,$sp,%d # allocate locals\n\n", -size);
            ...
            // pop stack
            emitInstruction(out, "addiu $sp,$sp,%d # free locals\n\n", size);
            break;
        }
    ...
    bool writeDebug, FILE* source)
{
    ...
    emitComment(out, "Instruction Count: %d\n", resetInstrCnt());
